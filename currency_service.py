...
async def main_async():
    ...
    # Test a currency not in our DEFAULT_CURRENCY_IDS map
    try:
        # This call is expected to raise CurrencyNotFoundError immediately
        fake_price: str = await get_currency_price("خیالی", DEFAULT_CURRENCY_IDS)
        # The following line should ideally not be reached if the exception is raised correctly.
        logger.info(f"The price of خیالی is: {fake_price}")
    except CurrencyNotFoundError as e:
        logger.error(f"Could not find currency data for خیالی: {e}")
    except CurrencyAPIError as e:
        logger.error(f"API error when fetching price for خیالی: {e}")
    except CurrencyError as e:
        logger.error(f"An unexpected currency error occurred for خیالی: {e}")
